...
/// 的点加入到树中，持续 `n - 1` 轮（边的数量）。
/// 1. 默认所有节点到树的距离是i32::MAX
/// 2. 选一节点加入树中，计算剩余节点到树的距离，通过 松弛 操作更新距离
/// 3. 选出距离最近的节点加入树中。
/// 4. 树中有了新节点，用新节点重新计算剩余节点到树的距离（和旧节点的距离已经记录，无需重复计算），然后继续 松弛。
/// 5. 持续以上过程直到所有节点都选完，算法结束。
